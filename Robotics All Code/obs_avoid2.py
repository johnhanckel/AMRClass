...
def laserCallBack(data):
	...
	if (np.mean(left_ranges) <= 1.):
		if (np.mean(center_ranges)<=1):
			az_obsavoid=-0.65
		else:
			az_obsavoid=-0.45
	if (np.mean(right_ranges)<=1.):
		if (np.mean(center_ranges)<=1.):
			az_obsavoid = 0.45
		else:
			az_obsavoid=0.25
	if (np.mean(left_ranges)>1. and np.mean(right_ranges)>1 and np.mean(center_ranges)>1):
		az_obsavoid = 0
	'''
	if (np.mean(center_ranges)<=1.):
	...
	print([np.mean(left_ranges),np.mean(center_ranges),np.mean(right_ranges),az_obsavoid])
	'''
	if (np.mean(center_ranges)<=1.):
		if (np.mean(left_ranges)>np.mean(right_ranges)):
			az_obsavoid=0.35
