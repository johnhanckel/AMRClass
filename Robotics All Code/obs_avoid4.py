...
def laserCallBack(data):
	...
	ll = [x for x in ll if x==x]
	cl = [x for x in cl if x==x]
	cr = [x for x in cr if x==x]
	rr = [x for x in rr if x==x]
	ll_mean = np.mean(ll)
	cl_mean = np.mean(cl)
	cr_mean = np.mean(cr)
	rr_mean = np.mean(rr)

	if (ll_mean<=d_avoid or ll_mean!=ll_mean):
		obs_avoid = True
		if (cl_mean<=d_avoid or cl_mean!=cl_mean):
			az_obsavoid = -1*hard_turn
		else:
			az_obsavoid = -1*soft_turn
	if (rr_mean<=d_avoid or rr_mean!=rr_mean):
		obs_avoid = True
		if (cr_mean<=d_avoid or cr_mean!=cr_mean):
			az_obsavoid = hard_turn
		else:
			az_obsavoid = soft_turn
	if (cr_mean<=d_avoid or cr_mean!=cr_mean):
		az_obsavoid = hard_turn
		obs_avoid = True
	if (cl_mean<=d_avoid or cl_mean!=cl_mean):
		az_obsavoid = -1*hard_turn
		obs_avoid = True
	if (ll_mean>d_avoid and cl_mean>d_avoid and cr_mean>d_avoid and rr_mean>d_avoid):
		az_obsavoid = 0
		obs_avoid = False
	
	
	print("[{:0.2f},{:0.2f},{:0.2f},{:0.2f}],{:0.2f}".format(ll_mean,cl_mean,cr_mean,rr_mean,az_obsavoid))
	'''
